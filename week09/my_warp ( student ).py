...
def my_LS(matches, kp1, kp2):
    ...
    for i in range(0, length):
        init_A.append([kp1[matches[i].queryIdx].pt[0], kp1[matches[i].queryIdx].pt[1], 1, 0, 0, 0])
        init_A.append([0, 0, 0, kp1[matches[i].queryIdx].pt[0], kp1[matches[i].queryIdx].pt[1], 1])
        init_B.append(kp2[matches[i].trainIdx].pt[0])
        init_B.append(kp2[matches[i].trainIdx].pt[1])

    A = np.asarray(init_A)
    B = np.asarray(init_B)

    X = np.transpose(np.array([np.dot(np.dot(np.linalg.inv(np.dot(np.transpose(A), A)), np.transpose(A)), np.transpose(B))]))
...
def my_forward(img, X):
    ...
    X = np.append(np.transpose(X), [0, 0, 1])
    M = np.transpose(X).reshape(3, 3)

    for i in range(y):
        for j in range(x):
            '''M과 좌표 (x,y)를 이용해 새로운 좌표인 newX, newY를 구하세요.'''
            res = np.dot(M, [j, i, 1])
            newX = int(round(res[0]))
            newY = int(round(res[1]))
...
def my_backward(img, X):
    ...
    for i in range(y):
        for j in range(x):
            res = np.dot(np.linalg.inv(M), [j, i, 1])  # 역행렬

            if res[1] < y and res[0] < x and res[1] > 0 and res[0] > 0:
                result[i, j, :] = my_bilinear(img, res[0], res[1])
                count[i, j, :] += 1
