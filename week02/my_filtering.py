...
src = cv2.imread('D:\\py_data\\big_shrimp.jpg', 0) #TA 사용 이미지 ( 4000 x 3000 )
gaus2D = my_getGKernel((51,51), 13)
gaus1D = my_getGKernel((1,51), 13)

start = time.perf_counter() # 시간 측정
img2D = gaus_filtering(src, gaus2D,boundary = ) #boundary = 0, 1, 2 골라서 사용. 미입력시 0 (zero-padding)
end = time.perf_counter()
print(end-start)

start = time.perf_counter()
img1D = gaus_filtering(src, gaus1D, boundary = )
img1D = gaus_filtering(img1D, gaus1D.T, boundary = )
end = time.perf_counter()
print(end-start)
